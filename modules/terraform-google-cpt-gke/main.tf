...
locals {
  ...
  master_authorized_networks = concat(var.master_authorized_networks, [
    for bastion in google_compute_instance_from_template.bastion_vm:
    {
      cidr_block   = "${bastion.network_interface.0.network_ip}/32"
  ...
  cluster_endpoint           = (length(google_container_cluster.primary.private_cluster_config) > 0) ? google_container_cluster.primary.private_cluster_config.0.private_endpoint : google_container_cluster.primary.endpoint
  cluster_peering_name       = (length(google_container_cluster.primary.private_cluster_config) > 0) ? google_container_cluster.primary.private_cluster_config.0.peering_name : null
  ...
  workload_identity_enabled                  = ! (local.identity_namespace == null || local.identity_namespace == "null")
  cluster_workload_identity_config = ! local.workload_identity_enabled ? [] : local.identity_namespace == "enabled" ? [{
    identity_namespace = "${var.project_id}.svc.id.goog" }] : [{ identity_namespace = local.identity_namespace
  }]
  # BETA features
